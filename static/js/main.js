...
function init_calendar(date) {
    ...
    for(let i=0; i<35+first_day; i++) {
        ...
        if(i < first_day || day > day_count) {
            ...
            let curr_date = $("<td class='table-date' style='color:blue;'>"+day+"</td>");
            ...
            console.log(curr.date.click({events: events, month: months[month], day:day}, date_click))
            row.append(curr_date);
        }
    }
...
function new_event(event, year) {
    ...
    $("#ok-button").unbind().click({date: event.data.date}, function() {
        let date = event.data.date;
        console.log('this is event year', event.data.year)
...
// function new_event_json(comment, date, day) {
//     let hours = parseInt($('#hours'))
//     let min = parseInt($('#minutes'))
//     let time_entry = hours + ":" + minutes
//     let event = {
//         "comment": comment,
